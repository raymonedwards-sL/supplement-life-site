...
export function Roadmap({ phases, weeklyCheckIn }: RoadmapProps) {
  return (
    <div className="rounded-2xl border border-navy/10 bg-white/70 p-6 sm:p-8">
      <p className="text-xs font-semibold uppercase tracking-wide text-navy/50">Your 90-Day Roadmap</p>
      ...
      <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-3">
        {phases.map((phase) => (
          <div key={phase.label} className="rounded-xl bg-navy/5 p-5">
            ...
            <p className="mt-2 text-sm leading-relaxed text-navy/70">{phase.focus}</p>
            <ul className="mt-3 flex flex-col gap-1.5">
              {phase.milestones.map((m, i) => (
                <li key={i} className="flex items-start gap-2 text-xs leading-relaxed text-navy/60">
      ...
      <div className="mt-6 border-t border-navy/10 pt-5">
        <p className="text-xs font-semibold uppercase tracking-wide text-navy/50">Your Weekly Check-In</p>
        <p className="mt-1.5 text-xs text-navy/50">
          Priority marker: <span className="font-semibold text-navy/75">{weeklyCheckIn.priorityMarkerQuestion}</span>
        </p>
