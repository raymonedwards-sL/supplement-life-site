 ...
 * regardless of any LIFE Assessment purchase on file — this preserves the
 * pre-existing "a refund revokes portal access" behavior rather than
 * letting a live assessment window become a backdoor around a refund.
 */
...
export function resolveIntakeAccess({
  ...
  if (lifeAssessmentPurchasedAt) {
    const purchasedAtMs = new Date(lifeAssessmentPurchasedAt).getTime();
    const windowMs = LIFE_ASSESSMENT_RETAKE_WINDOW_DAYS * 24 * 60 * 60 * 1000;
    if (Date.now() - purchasedAtMs <= windowMs) {
      return { allowed: true };
    }
  ...
  // only ever created by the Stripe webhook after one of the two real
  // purchases (see components/LoginPrompt.tsx's file comment), so every
  // real account should have at least one of the two rows checked above.
